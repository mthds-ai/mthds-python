...
- **One `RunResult`, two base fields.** Both `execute` and `start` answer with `RunResult{pipeline_run_id, pipe_output}` — `pipeline_run_id` mandatory and server-authoritative; `pipe_output` present on a completed `execute`, absent on `start`. Run states, timestamps, and output naming are implementation extension fields (`extra="allow"`, read via `model_extra`), never named by the SDK. There is no `StartAck` / `RunState`, and the request side carries no `pipeline_run_id` (a client-supplied run id is an extension arg, passed through `extra`).
