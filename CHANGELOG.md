...
- **Breaking: a top-level input-form field stating `required` and `presence` incoherently now fails the parse.** `docs/spec/input-form-descriptor.md` derives a top-level field's `required` as `presence != "optional"`, and `PipeInputFormDescriptor` now enforces that relationship instead of documenting it: `required: true` beside `presence: "optional"`, or `required: false` beside `plain`/`force`, is rejected. The check reuses `PresenceMarker.is_optional`, so the three-way marker logic stays stated in one place.
- **Breaking: an explicit `null` spelling absence on an input-form wire slot now fails the parse.** The wire rule was already "absent, never `null`" on the emission side; the intake now holds it too, instead of silently canonicalizing `title: null`, `refines: null`, or a variable list's `item_count: null` on the round trip. `default_value` is the one carve-out — its `null` IS "no default" per the page and still parses. The check binds raw wire mappings alone, walked from `PipeInputFormDescriptor` where every wire node arrives, so programmatic construction (`TextField(..., title=maybe_title)` with `None`) stays the supported idiom — a node-level `mode="before"` validator cannot tell a wire mapping from constructor keywords, which is why the rejection does not live on the node base.

- **Breaking: the input-form descriptor's field union splits into a nameless item shape and a named field shape.** `mthds/protocol/input_form.py` used to model `name` as `name: str | None = None` on the common base, which neither required it where the standard requires it nor forbade it where the standard forbids it. `docs/spec/input-form-descriptor.md` gives `name` as applicable "on every node except a `list`'s `item`", which "has no authored name and carries no `name` member at all — the index labels items, and a sentinel would be a value two producers could pick differently". Both halves of that rule are now structural rather than unstated:
  - `InputFormItem` is the nameless union — the shape of a `list`'s `item` — over per-kind models named `TextItem`, `ProseItem`, `DateItem`, `NumberItem`, `BooleanItem`, `EnumItem`, `DocumentItem`, `ImageItem`, `ObjectItem`, `ListItem` and `UnknownItem`, none of which declares `name`. Since every model is a closed shape, an item carrying a `name` is an unknown member and fails the parse.
  - `InputFormField` stays the named union, and each of its per-kind models (`TextField`, `ObjectField`, …) is now its item counterpart plus a required `name: str`. A named position missing its name fails the parse, and a consumer reading `field.name` gets a `str` rather than something to narrow. This is the pydantic spelling of the TypeScript mirror's `InputFormField = InputFormItem & { name: string }`, so the two clients enforce the rule the same way.
  - Which layer applies is decided by position: `PipeInputFormDescriptor.fields` and `ObjectField.fields` hold `InputFormField`, `ListField.item` holds `InputFormItem`.
  - Renamed with the split: `InputFormFieldBase` is now `InputFormItemBase` and `TextValuedFieldBase` is now `TextValuedItemBase`; the new `InputFormNamedNode` is the layer that adds the name. The wire is unchanged — `kind` and `name` still lead a serialized node, restored by the common serializer now that a named model declares `name` last.
- **The protocol parity fixtures are recaptured from the corrected engine.** `tests/fixtures/protocol/` now holds `pipelex` at checkout `bdd853c41` (its `pyproject.toml` still reads 0.53.0; that release is not cut yet), replacing the 0.53.0 capture taken at `bc97dad0b`. The same bytes land in `mthds-js`. The previous capture's three divergences are all fixed in the engine, so the capture has no known divergence from the pages, and the suite asserts each rule positively instead of recording it as drift: a `list`'s `item` carries no `name`, description-only concepts no longer carry a fabricated `refines: ["native.Text"]`, and `native.Date` and `native.Html` land on the page's `object` arm with `native.Html`'s `css_class` optional. The fixture README explains why the `json_schema` values keep pydantic's auto-generated titles — the standard leaves that projection to the producer, and `concept_ref` is the authoritative identity.
