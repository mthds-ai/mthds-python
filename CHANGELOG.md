...
- **`mthds.protocol.options` — the run-source argument surface:** predicates that decide which combinations of `pipe_code`, `mthds_contents`, the bundle encodings `files` / `bundle_b64`, `method_ref` and `method_id` may be sent together (`assert_exclusive_run_sources`, `has_bundle_payload`, `assert_method_ref_pairs_with_nothing`), plus the selector normalization `normalized_selector` / `run_selector_extensions`, which refuses a non-string selector. Nothing in this package calls them, so no client behaviour changes: `MthdsAPIClient` still forwards the keys inside `extra` without interpreting them.

### Changed

- **`ConceptAbstract` names a concept and nothing more, and `StuffAbstract` serializes `concept` as the ref string (Breaking)**: the protocol's concept model keeps `code`, `domain_code` and the `concept_ref` property — `description`, `structure_class_name` and `refines` are gone, since a concept's definition belongs to the library the method loads and never travels beside a stuff, and a runtime that needs one of them in memory declares it on its own subclass. `StuffAbstract` now carries a field serializer on `concept`, so every dump of a stuff emits `{"concept": "<domain>.<Code>", "content": …}` — the wire form the standard's [CLI I/O contract](https://mthds.ai/latest/spec/cli-io-contract/) requires — instead of leaving each runner to reduce the object itself. The dependency-contributed crate key `<package_address>::<domain>.<Code>` is a subclass's to produce, by overriding `StuffAbstract.serialize_concept` under that name: the protocol model holds no declaring package address. The dump is one-way, which is the point rather than an oversight — a `StuffAbstract`, a `WorkingMemoryAbstract` or a `PipeOutputAbstract` can no longer be validated back from its own dump, since the definition such a parse would need is exactly what no longer travels. A consumer that used to re-validate a dump resolves the concept by ref through the library the method loads, the way the input side already does. Which of the two override points a runtime picks decides what this SDK's own output path emits: `DictRunResultExecute.from_pipe_output` reduces a stuff by `concept_ref`, so a `concept_ref` override reaches it and a `serialize_concept` override does not.
...
- **`mthds.protocol.output_form` — the output-form descriptor.** `PipeOutputFormDescriptor` and `OutputForm` mirror the input-form descriptor over the same key set as `pipe_io_contracts` and `input_form`. The descriptor carries a single `field` rather than a `fields` list, since a pipe has exactly one output, and reuses `InputFormField` verbatim rather than declaring a second node union that could drift. It enforces the absence of the input-specific slot facts `presence` and `gating`, failing the parse when a producer supplies them, and states plurality by wrapping the field in a `list` node derived from the contract's multiplicity.
- **A CI gate on the shared protocol fixture corpus.** `.github/workflows/protocol-corpus-parity.yml` checks out `mthds-js` on every pull request and compares `tests/fixtures/protocol/` byte for byte, so a one-sided recapture fails the pull request that causes it instead of surfacing weeks later in `conformance`. It needs no secret — both mirrors are public — and compares against the sibling's branch of the same name when one exists, `dev` otherwise. The file is a vendored copy of `conformance/tests/mthds/fixtures/protocol_corpus_parity_workflow.yml`; edit the canonical copy and re-vendor.
- **`tests/unit/test_protocol_output_form.py`** — parity against the new `output_form.json` capture, covering the shared key set, the absence of both slot facts, and the plural wrap, which is the one rule a type cannot state and the one that fails silently.

### Changed

- **`PipeOutputContract.json_schema` is now required.** The output contract is a closed shape and the member has no default, so a payload from an earlier engine fails the parse. Where an input's schema describes what a caller sends — a plural slot's being a bare array — an output's describes what comes back, which is the concept's content model: a plural output resolves to the list content model rather than a bare array, and a fixed arm carries `minItems`/`maxItems` equal to `item_count`. Since `optional` already states the output may be absent, the schema describes the shape when present and carries no null arm. **(Breaking)**
- **The projection fixture corpus covers the natives whose payload shape the descriptor states openly.** A recapture from `pipelex` `dev` adds a pipe placing `native.JSON`, `native.Dynamic` and `native.Composite` at slot positions. `Dynamic` and `Composite` are structureless, so their node is `unknown` and both renderers agree on the empty mapping; `JSON` carries a required `json_obj`, which puts an unknown node at a required object-field position for the first time and declares a new divergence class, `unknown-empty-object` — the projection renders the empty object where the reference engine fills in a sample pair. `input_form.json`, `pipe_io_contracts.json` and `inputs_template/manifest.json` move with it, and `mthds-js` commits the identical bytes.
- **The corpus suite notices a capture that arrives half-formed.** `test_inputs_template_projection.py` now enforces two properties it always had but never asserted: the README's divergence bullets must match the manifest's declarations in both directions, so a retired class cannot leave its prose behind, and `input_form.json` and `pipe_io_contracts.json` must name the same set of pipes, since they are one capture and nothing outside the contracts file's own parse test read it.
