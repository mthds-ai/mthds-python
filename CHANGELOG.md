...
- **`mthds.protocol.output_form` — the output-form descriptor, mirroring the standard's new page.** `PipeOutputFormDescriptor` carries one `field`, not a `fields` list: a pipe has exactly one output where it may have many inputs, which is the only shape difference from its input twin and follows from the language. `OutputForm` is the `pipe_ref` → descriptor map, over the same key set as `pipe_io_contracts` and `input_form`.

  The module declares no node type. `field` is `InputFormField`, reused verbatim — an output is a concept reference exactly like an input is, so its kinds, its nesting and its constraints are the same questions with the same answers, and a second node union would be a second place for them to drift. What it adds is the enforcement of what an output does NOT carry: `presence` and `gating` are facts of an input slot, both already optional on the node so that a slotless node could exist, and a producer that fills them in with something plausible now fails the parse. The wire-null rule (`title: None` is absent, never `null`) is stated on the container for the same reason its input twin states it there — only at that level can a raw wire mapping be told from the keyword arguments of a programmatic construction.

- **`tests/unit/test_protocol_output_form.py`** — parity against the new `output_form.json` capture, and the rules a type cannot state: the shared key set, the absence of both slot facts, and the **plural wrap** (a plural output described as a `list`, a single one not), which is the one place producing this artifact is real work rather than reuse and the one that fails silently. It also asserts what the corpus must reach — both plural arms — since a capture with no plural output could not have caught the wrap being skipped at all.

- **A CI gate on the shared protocol fixture corpus.** `tests/fixtures/protocol/` is committed byte-identically here and in `mthds-js`, and until now nothing in either repo checked that: the only comparison lived in `conformance`, where it ran on a `conformance` pull request rather than on the one that moved the corpus. A recapture landed here without its `mthds-js` twin was therefore caught weeks later by an unrelated change, and the pairing held only because prose in two pull request bodies asked for it. `.github/workflows/protocol-corpus-parity.yml` now checks `mthds-js` out on every pull request and compares the two trees, so a one-sided recapture fails the pull request that causes it.

  It needs no secret — both mirrors are public, so the sibling checkout is unauthenticated and the job runs on a pull request from a fork exactly as it does on one from a branch. It compares against the sibling's branch of the same name when one exists and against `dev` otherwise, because a recapture lands as a paired pull request in both mirrors at once and comparing each against the other's `dev` would leave both red until the first merged. The file is a **vendored copy** of `conformance/tests/mthds/fixtures/protocol_corpus_parity_workflow.yml` and is byte-identical in both mirrors, which is why it derives which repo it is running in rather than naming one; `make check-fixture-drift` in `conformance` fails when a copy drifts or goes missing, so edit the canonical copy and re-vendor rather than editing this one in place.

### Changed

- **`PipeOutputContract.json_schema` — the payload's schema (breaking).** The output contract was a closed shape with four members and no schema, on the reasoning that a run's payload is the run's own result. That answers *what did this run produce?*, correctly not a contract's business, rather than the question a consumer asks: *what shape will it be?* — which is declared in the source and knowable before any run.

  **It is the PAYLOAD's schema, and that is where its rule departs from the input twin's.** An input's describes what a caller *sends*, so a plural slot's is a bare array. An output's describes what *comes back*, which is the concept's content model — so a `native.Text` output's is the object declaring `text`, and a `Concept[]` output's is the list content model rather than a bare array. On the fixed arm the element array carries `minItems`/`maxItems` equal to `item_count`; since `optional` already states the output may be absent, the schema describes the shape when present and carries no null arm.

  Breaking because the shape is closed and the member is required: a payload from an earlier engine fails the parse, which is the intended behaviour for a minor version of the artifact.
