...
- **Docs are library-first; the phantom CLI reference is gone.** `mthds-python` is a library with no command line — the `mthds` CLI ships as the npm package (mthds-js). README is rewritten around the Python API (`MthdsAPIClient`, `start_and_wait`, config resolution) and `CLI.md` (which wrongly claimed `pip install mthds` put `mthds` on your PATH) is removed.
- **`mthds/runners/` reorganized into per-runner subpackages.** The API runner is `mthds.runners.api` — `client.py` (`MthdsAPIClient`), `models.py` (Dict wire models), `runs.py` (hosted polling models), `exceptions.py` (API auth + lifecycle errors). The local CLI runner is `mthds.runners.pipelex.runner` (`PipelexRunner`). The `create_runner` factory (`registry.py`) is removed — it had no production callers; construct the runner you need directly.
- **Domain models relocated: non-Dict abstracts → `mthds/protocol/`, Dict serialization + API runner → `mthds/runners/api/`.** The protocol's abstract data shapes (`ConceptAbstract`, `StuffAbstract`/`StuffContentAbstract`, `WorkingMemoryAbstract`, `PipeOutputAbstract`, `PipelineInputs`) now live beside the protocol in `mthds.protocol.{concept,stuff,working_memory,pipe_output,pipeline_inputs}` — the `mthds.models` package is gone. Their Dict-serialized counterparts (`DictStuffAbstract`, `DictWorkingMemoryAbstract`, `DictPipeOutputAbstract`, `DictRunResult`) and the API runner moved under `mthds.runners.api` — `mthds.runners.api.models` and `mthds.runners.api.client` (`MthdsAPIClient`, was `mthds.runners.api_runner`). Layering is now strict: `mthds.protocol` depends on nothing under `mthds.runners`.
- **`mthds/client/` split into `mthds/protocol/` + `mthds/runners/`.** The protocol contract now lives in its own package — `mthds.protocol.protocol` (the `MTHDSProtocol` interface), `mthds.protocol.models` (all wire models mirroring `mthds-protocol.openapi.yaml`, merging the former `pipeline.py` + `protocol_models.py`), `mthds.protocol.exceptions` (`PipelineRequestError`) — and every implementation lives under `mthds/runners/`: `mthds.runners.api.client` (`MthdsAPIClient`), `mthds.runners.api.runs` (hosted run-lifecycle models), `mthds.runners.api.exceptions` (runner errors), alongside the `mthds.runners.pipelex.runner`. The `mthds.client` package is gone; class names are unchanged.
- **Discovery/validation response models slimmed to the protocol's base fields, extension-open.** `VersionInfo` = `{protocol_version, runner_version}`; `ValidationReport` declares no body fields (the 200 IS the verdict); `ModelDeck` = `models[{name, type}]`; `RunResults` = `{pipeline_run_id, main_stuff}`; `RunPublic` = the base run record. Everything else a server returns (implementation identification, routing metadata, validation artifacts, result graphs, run identity) passes through as extension attributes (`extra="allow"`, read via attributes or `model_extra`) — the response-side mirror of the request-side `extra`.
- **Extension args removed from the SDK; generic `extra` passthrough added.** The SDK carries the MTHDS Protocol's basic arguments only — server-specific extension args never appear in it, not even as convenience params. Both `execute` and `start` gain a generic `extra` mapping: any server-specific arg merges into the request body as a top-level property and the server that defines it handles it (protocol args inside `extra` are rejected client-side).
...
- **The request model is gone entirely — the protocol defines no request *model*.** `RunRequest` (and the transient `StartRequest`) are deleted, along with their factory helpers (`from_working_memory`, `from_body`) and the run response's `from_api_response`. The "at least one of pipe_code/mthds_contents" check was API-runner policy, not wire shape — it lives in the runner's `execute`/`start` guards. The runner now assembles the request body as a plain mapping of the protocol's basic args + the `extra` passthrough and serializes it with `pydantic_core.to_json` (which handles pydantic `inputs`); a server that wants a typed request model (e.g. pipelex-api) defines its own.
...
- **Request-side `pipeline_run_id` dropped from the protocol surface.** It is not a protocol argument: `MTHDSProtocol.start` and `MthdsAPIClient.start`/`start_and_wait` no longer name it. A server that accepts a client-supplied run identifier defines it as an extension arg — pass it through `extra` like any other. The `pipeline_run_id` in responses (run results, run-lifecycle reads) is unchanged and always authoritative.
