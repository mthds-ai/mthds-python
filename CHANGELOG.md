...
- **`mthds.protocol.options` — the run-source argument surface:** the predicates that decide which combinations of `pipe_code`, `mthds_contents`, the pipelex-api bundle encodings (`files` / `bundle_b64`), a published address (`method_ref`) and the hosted catalog id (`method_id`) may be sent together, plus the boundary normalization a method selector goes through (`normalized_selector`, `run_selector_extensions`), where a non-string is refused rather than silently dropped or forwarded to a server 422 by a client that takes these arguments by name. The rule is an invariant of the request rather than of any one runner, so it now has one Python definition for clients to enforce from instead of re-deriving it. **The definitions ship; nothing enforces from them yet, and no client behaviour changes.** `MthdsAPIClient` calls none of them — it is a client for any MTHDS-compliant runner, so it keeps forwarding the keys inside `extra` without interpreting them — and `PipelexAPIClient` in `pipelex-sdk` still carries its own private copies, which it can replace once it can depend on a release carrying this module. The module is the twin of `mthds-js`'s `protocol/options.ts`, matching its two exported predicates message for message; it does not mirror either server, which word the same refusals differently from each other.
