...
- **`RunRequest.validate_request` validator removed; `StartRequest` collapsed into `RunRequest`.** The "at least one of pipe_code/mthds_contents" check was API-runner policy baked into a pure wire model — it lives in the runner's `execute`/`start` (where it belongs), not the model. `StartRequest` carried no fields beyond `RunRequest`, so `/start` now uses the `RunRequest` body directly.
- **Factory helpers stripped from the protocol models — pure wire shapes only.** `RunRequest.from_working_memory`, `RunRequest.from_body`, and the run response's `from_api_response` are gone: they were SDK/runner conveniences, not protocol. Construct with the model directly or `model_validate` (which is what the API runner now does); a server that supports legacy body shapes (e.g. singular `mthds_content`) owns that parsing itself.
