...
class TestRunners:
    ...
    def test_create_runner_api_explicit(self) -> None:
        """Passing RunnerType.API explicitly returns the MthdsAPIClient itself (parity D8 — no wrapper)."""
        runner = create_runner(RunnerType.API)
        assert isinstance(runner, MthdsAPIClient)
    ...
    def test_create_runner_pipelex_falls_back_to_api(self, mocker: MockerFixture) -> None:
        """When pipelex is not on PATH, RunnerType.PIPELEX falls back to the API client."""
    ...
    def test_create_runner_none_reads_credentials_api(self, mocker: MockerFixture) -> None:
        """When runner_type is None, reads from credentials; 'api' yields the API client."""
        mocker.patch(
    ...
    def test_create_runner_none_invalid_configured_value_falls_back_to_api(self, mocker: MockerFixture) -> None:
        """When credentials contain an invalid runner value, fall back to the API client."""
        mocker.patch(
            "mthds.runners.registry.load_credentials",
    ...
    @pytest.mark.parametrize(
        ...
        [
            (RunnerType.API, MthdsAPIClient),
            (RunnerType.PIPELEX, MthdsAPIClient),  # pipelex not on PATH -> fallback
        ],
    )
    def test_create_runner_explicit_types_no_pipelex(
        ...
        """Both explicit types return the API client when pipelex is not on PATH."""
        mocker.patch("mthds.runners.registry.shutil.which", return_value=None)
